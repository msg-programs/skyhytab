...
import com.msgprograms.skyhytab.widget.Widget;
...
public class Screen {
    ...
    private enum ScreenType {
        ...
        , GUEST_ISLAND, HOME_ISLAND, CRIMSON_ISLE, DUNGEON_HUB, FARMING_ISLAND, PARK, DWARVEN_MINES, CRYSTAL_HOLLOWS,
        END, GOLD_MINE, DEEP_CAVERNS, HUB, SPIDER_DEN, JERRY, NONE
    }
    ...
    public void stackWidgetsH(Widget... list) {
        int compHeight = -5; // one padding less
        for (Widget wid : list) {
    ...
    public void stackWidgetsW(Widget... list) {
        // TODO not centered
        int compWidth = -5; // one padding less
        for (Widget wid : list) {
    ...
    public static Screen getCorrect(int w, int h, List<PlayerListEntry> ple, Text footer) {
        if (SkyHyTabMain.genericTgl.isPressed()) {
            return new GenericInfoScreen(w, h, ple, footer);
        } else if (SkyHyTabMain.mapTgl.isPressed()) {
            return Screen.correctMapScrn(w, h, ple, footer);
        } else if (SkyHyTabMain.playerTgl.isPressed()) {
